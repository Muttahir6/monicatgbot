...
def new_fed(bot: Bot, update: Update, args: List[str]):
    if len(args) >= 1:
        ...
        if not sql.search_fed_by_name(fed_name) == False:
                update.effective_message.reply_text(tld(chat.id, "Federation already exists with this name, Please change name!"))
                return

        print(fed_id)
        sql.new_fed(user.id, fed_name, fed_id)
        update.effective_message.reply_text("*Successfully created federation!*"\
                        "\nName: `{}`"\
                        "\nID: `{}`"
...
def del_fed(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
                ...
                update.effective_message.reply_text(tld(chat.id, "Successfully Deleted!"))
        else:
                update.effective_message.reply_text(tld(chat.id, "Please write fed id to remove!"))
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        update.effective_message.reply_text(tld(chat.id, "Chat joined to this federation!"))
    else:
...
def leave_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if sql.chat_leave_fed(chat.id) == True:
        update.effective_message.reply_text(tld(chat.id, "Left from this fed!"))
    else:
...
def user_join_fed(bot: Bot, update: Update, args: List[str]):
        ...
        if not sql.search_user_in_fed(fed_id, user_id) == False:
                update.effective_message.reply_text(tld(chat.id, "I can't promote a user which is already a fed admin! But I can demote him."))
                return

        res = sql.user_join_fed(fed_id, user_id)
        update.effective_message.reply_text(tld(chat.id, "Promoted in federation!"))
...
def user_demote_fed(bot: Bot, update: Update, args: List[str]):
        ...
        if sql.search_user_in_fed(fed_id, user_id) == False:
                update.effective_message.reply_text(tld(chat.id, "I can't demote a user which is not even a fed admin! If you wanna bring him to tears, promote him and then demote."))
                return
...
def fed_info(bot: Bot, update: Update, args: List[str]):
        ...
        if not fed_id:
            update.effective_message.reply_text(tld(chat.id, "This group is not linked to any federation!"))
            return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    message.reply_text(tld(chat.id, "Starting federation ban!"))
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        update.effective_message.reply_text(tld(chat.id, "Successfully set rule's for this fed!"))
    else:
...
def welcome_fed(bot, update):
    ...
    if not fban == False:
        update.effective_message.reply_text(tld(chat.id, "If this user is banned in current federation! I will remove him."))
